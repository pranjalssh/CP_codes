...
void crt(mpz_t res, const vector<int> &divs, const vector<int> &mods) {
	mpz_t div, mod, d, ndiv, nmod, t;
	mpz_init_set_si(div, divs[0]);
	mpz_init_set_si(mod, mods[0]);
	mpz_t apr[3];
	rep(i, 3) mpz_init(apr[i]);
	mpz_init(d);
	mpz_init(ndiv);
	mpz_init(nmod);
	mpz_init(t);
	reu(i, 1, divs.size()) {
		mpz_set_si(d, divs[i]);
		mpz_gcdext(apr[0], apr[1], apr[2], div, d);
		mpz_set(ndiv, div);
		mpz_mul_si(ndiv, ndiv, divs[i]);
		mpz_divexact(ndiv, ndiv, apr[0]);
		mpz_set(nmod, apr[1]);
		mpz_set_si(t, mods[i]);
		mpz_sub(t, t, mod);
		mpz_mul(nmod, nmod, t);
		mpz_divexact(nmod, nmod, apr[0]);
		mpz_mul(nmod, nmod, div);
		mpz_add(nmod, nmod, mod);
		mpz_mod(nmod, nmod, ndiv);
		mpz_set(div, ndiv);
		mpz_set(mod, nmod);
	}
	mpz_set(res, mod);
